...
let numbersOfString = (str) => Array.to_list(Array.map(int_of_string, Js_string.split("\t", str)));

let linesOfString = (str) => Array.to_list(Js_string.split("\n", str));
...
let checksum = (lines) => {
  let diffs = List.map((x) => numbersOfString(x) |> biggestGap, linesOfString(lines));
  List.fold_left((sum, (low, high)) => sum + high - low, 0, diffs)
};

/* Js.log(checksum(input)) */
/* Part 2 */
let findEvenlyDivisible = (nums: array(int)) => {
  let div = ref(0);
  for (i in 0 to Array.length(nums) - 2) {
    for (j in i + 1 to Array.length(nums) - 1) {
      let a = nums[i];
      let b = nums[j];
      if (a mod b === 0) {
        div := a / b
...
let evenlyDivides = (lines: string) =>
  List.fold_left(
    (sum, line) => sum + (numbersOfString(line) |> Array.of_list |> findEvenlyDivisible),
    0,
    linesOfString(lines)
  );

Js.log(evenlyDivides(input));
